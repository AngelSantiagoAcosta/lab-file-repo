...
def apply_validation_checks(df): #takes dataframe, returns dataframe with new validation columns with pass/failed reason.
    ...
    df["Validation 24"] = np.where(((df["Origin"] == "Sysmex XN-1000")) & (df["Analyte"].str.contains("Fragments?") == False) (df["TestResultFlags"].isin(["W","A"])) & (df["Repeat"] != "Y"),"Missing Repeat = 'Y'","passed")
